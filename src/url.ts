...
import type { PathParams, Query } from './types'

/*
 *   PATH PARAMS
 ***************************************************************************************************/
/**
 * A placeholder starts a path segment: `/users/:id`, or `:id` at the very
 * front. A colon anywhere else is literal, which keeps custom-method URLs like
 * `/v1/models/gemini-pro:generateContent` and credentials in an authority
 * (`//user:pass@host`) working. Write `::name` for a literal segment that has
 * to begin with a colon.
 */
const PARAM_PATTERN = /(^|\/)(::?)([A-Za-z_][A-Za-z0-9_]*)/g

/**
 * Substitutes `:name` placeholders. A missing value is a `CONFIG` error rather
 * than a request to a literal `/users/:id`, which would come back as a
 * confusing 404 far from the actual mistake.
 */
export function applyPathParams(path: string, params: PathParams | undefined): string {
	// Only the path is scanned. A colon is ordinary inside a query value
	// (`?q=label:urgent`), and treating one as a missing placeholder would make
	// the request unsendable.
 ...
 ***************************************************************************************************/
/**
 * Appends query values in the order given. `undefined` and `null` are dropped
 * rather than serialised as the strings "undefined" and "null"; arrays repeat
 * the key.
 */
